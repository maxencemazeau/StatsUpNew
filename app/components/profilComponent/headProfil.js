    ...
    return (
        <View style={{ padding: 20 }}>
            ...
            <View style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', marginTop: 5 }}>
                ...
                <Text style={{ fontSize: 20, marginTop: 10, color: "black" }}>{userProfil?.FirstName} {userProfil?.LastName}</Text>
                {myUserID !== userProfil?.UserID && <Button style={{ bgcolor: "#DD7A34", marginTop: 10 }}>Follow</Button>}
            </View>
        </View>
    )
}
